...
public class CategoryImportQueueProcessingService(
    QueueServiceClient queueServiceClient,
    ILogger<CategoryImportQueueProcessingService> logger,
    IServiceScopeFactory scopeFactory) : BackgroundService
{
    private readonly QueueClient _client =
        queueServiceClient.GetQueueClient(skestock.Shared.Services.CategoryImportQueue);
    ...
    private async Task ProcessMessageAsync(QueueMessage message, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (response is IResultBase { IsSuccess: false } failedResult)
            {
                ...
                logger.LogError(
                    "Category import message {Id} failed: {Errors}",
    ...
    {
        var poisonQueue =
            queueServiceClient.GetQueueClient($"{skestock.Shared.Services.CategoryImportQueue}-poison");
