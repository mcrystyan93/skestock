...
public class GetAllStockBatchesQueryValidator : AbstractValidator<GetAllStockBatchesQuery>
{
    ...
    public GetAllStockBatchesQueryValidator()
    {
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllStockBatchesQuery query)
    {
        var cursorState = CursorCodec<StockBatch>.Decode(query.Cursor);
        if (cursorState is null)
            return true; // Malformed cursor is already reported by the InvalidCursor rule above.

        var effectiveSort = DynamicSortBuilder<StockBatch>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<StockBatch>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<StockBatch>.Decode(cursor);
