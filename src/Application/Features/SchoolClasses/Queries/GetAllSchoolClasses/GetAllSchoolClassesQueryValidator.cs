...
public class GetAllSchoolClassesQueryValidator: AbstractValidator<GetAllSchoolClassesQuery>
{
    ...
    public GetAllSchoolClassesQueryValidator()
    {
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllSchoolClassesQuery query)
    {
        var cursorState = CursorCodec<SchoolClass>.Decode(query.Cursor);
        if (cursorState is null)
            return true; // Malformed cursor is already reported by the InvalidCursor rule above.

        var effectiveSort = DynamicSortBuilder<SchoolClass>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<SchoolClass>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<SchoolClass>.Decode(cursor);
