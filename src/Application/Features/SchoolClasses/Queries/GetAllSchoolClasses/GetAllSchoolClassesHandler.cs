    ...
    : IRequestHandler<GetAllSchoolClassesQuery, Result<PaginatedResponse<SchoolClassDto>>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        query = FilterQueryBuilder<SchoolClass>.Apply(
            query,
            request.Filters,
            FilterConfiguration,
        ...
        if (!string.IsNullOrWhiteSpace(request.SearchTerm))
        {
            ...
            query = TextSearchCollation.IsSqlServer(dbContext.Database)
                ? query.Where(c =>
                    EF.Functions.Collate(c.Name, TextSearchCollation.AccentInsensitive).Contains(term))
                : query.Where(c => c.Name.Contains(term));
        }
