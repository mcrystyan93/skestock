...
public class GetAllLocationsQueryValidator: AbstractValidator<GetAllLocationsQuery>
{
    ...
    public GetAllLocationsQueryValidator()
    {
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllLocationsQuery query)
    {
        var cursorState = CursorCodec<Location>.Decode(query.Cursor);
        if (cursorState is null)
            return true; // Malformed cursor is already reported by the InvalidCursor rule above.

        var effectiveSort = DynamicSortBuilder<Location>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<Location>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<Location>.Decode(cursor);
