...
public class GetAllItemImportsQueryValidator : AbstractValidator<GetAllItemImportsQuery>
{
    ...
    public GetAllItemImportsQueryValidator()
    {
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllItemImportsQuery query)
    {
        var cursorState = CursorCodec<ItemImport>.Decode(query.Cursor);
        if (cursorState is null)
            return true; // Malformed cursor is already reported by the InvalidCursor rule above.

        var effectiveSort = DynamicSortBuilder<ItemImport>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<ItemImport>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<ItemImport>.Decode(cursor);
