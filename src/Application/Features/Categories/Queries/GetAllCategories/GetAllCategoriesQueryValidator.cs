...
public class GetAllCategoriesQueryValidator: AbstractValidator<GetAllCategoriesQuery>
{
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllCategoriesQuery query)
    {
        var cursorState = CursorCodec<Category>.Decode(query.Cursor);
        if (cursorState is null)
            return true; // Malformed cursor is already reported by the InvalidCursor rule above.

        var effectiveSort = DynamicSortBuilder<Category>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<Category>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<Category>.Decode(cursor);
