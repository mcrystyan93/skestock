...
public class GetAllGoodsReceiptImportsQueryValidator : AbstractValidator<GetAllGoodsReceiptImportsQuery>
{
    ...
    public GetAllGoodsReceiptImportsQueryValidator()
    {
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllGoodsReceiptImportsQuery query)
    {
        var cursorState = CursorCodec<GoodsReceiptImport>.Decode(query.Cursor);
        if (cursorState is null)
            return true; // Malformed cursor is already reported by the InvalidCursor rule above.

        var effectiveSort = DynamicSortBuilder<GoodsReceiptImport>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<GoodsReceiptImport>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<GoodsReceiptImport>.Decode(cursor);
