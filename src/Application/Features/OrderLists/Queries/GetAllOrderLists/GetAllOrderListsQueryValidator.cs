...
public class GetAllOrderListsQueryValidator : AbstractValidator<GetAllOrderListsQuery>
{
    ...
    public GetAllOrderListsQueryValidator()
    {
    ...
    private static bool HaveCursorMatchingRequestedSort(GetAllOrderListsQuery query)
    {
        var cursorState = CursorCodec<OrderList>.Decode(query.Cursor);
        if (cursorState is null)
            return true;

        var effectiveSort = DynamicSortBuilder<OrderList>.BuildEffectiveSort(query.Sort, SortConfiguration);
        return CursorCodec<OrderList>.MatchesSort(cursorState, effectiveSort);
    }
    ...
    private static bool ValidateCursor(string? cursor)
    {
        ...
        var decoded = CursorCodec<OrderList>.Decode(cursor);
