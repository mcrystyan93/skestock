...
public class EnableItemCommandHandlerTests
{
    ...
    [Test]
    public async Task Handle_WithInactiveItem_SetsIsActiveTrue()
    {
        var (context, item) = await CreateContextWithItemAsync(isActive: false);
        await using var _ = context;
        var handler = new EnableItemCommandHandler(context);

        var result = await handler.Handle(new EnableItemCommand { Id = item.Id }, CancellationToken.None);
    ...
    {
        var (context, _) = await CreateContextWithItemAsync(isActive: false);
        await using var _disposable = context;
        var handler = new EnableItemCommandHandler(context);

        var result = await handler.Handle(new EnableItemCommand { Id = Guid.NewGuid() }, CancellationToken.None);
