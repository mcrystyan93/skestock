...
public class ProcessCategoryImportBatchCommandHandlerTests
{
    private static ProcessCategoryImportBatchTestDbContext CreateContext()
    {
        var options = new DbContextOptionsBuilder<ProcessCategoryImportBatchTestDbContext>()
            .UseInMemoryDatabase(Guid.NewGuid().ToString())
            .Options;

        return new ProcessCategoryImportBatchTestDbContext(options);
    }

    private static async Task<(ProcessCategoryImportBatchTestDbContext Context, CategoryImportBatch Import)>
        CreateContextWithProcessingImportAsync()
    {
        ...
        var import = CategoryImportBatch.Create(Guid.NewGuid(), null, [file.Id]);
        import.Files.Single().FileMetadata = file;
    ...
    public async Task Handle_WhenImportDoesNotExist_ReturnsNotFoundFailure()
    {
        ...
        var handler = new ProcessCategoryImportBatchCommandHandler(
            context, blob.Object, extraction.Object,
            NullLogger<ProcessCategoryImportBatchCommandHandler>.Instance,
            Options.Create(new ImportBatchOptions()));

        var result = await handler.Handle(
            new ProcessCategoryImportBatchCommand(Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task Handle_WhenExtractionSucceeds_StoresSuggestionsAndTransitionsToPendingReview()
    {
        ...
        var result = await handler.Handle(
            new ProcessCategoryImportBatchCommand(import.Id), CancellationToken.None);

        result.IsSuccess.ShouldBeTrue();

        var persisted = await context.CategoryImportBatches.SingleAsync(x => x.Id == import.Id, CancellationToken.None);
        persisted.Status.ShouldBe(CategoryImportBatchStatus.PendingReview);
    ...
    public async Task Handle_WhenExtractionSucceeds_DoesNotCreateCategories()
    {
        ...
        await handler.Handle(new ProcessCategoryImportBatchCommand(import.Id), CancellationToken.None);
    ...
    public async Task Handle_WhenExtractionIsUnprocessable_MarksImportFailedAndReturnsOk()
    {
        ...
        var persisted = await context.CategoryImportBatches.SingleAsync(x => x.Id == import.Id, CancellationToken.None);
        persisted.Status.ShouldBe(CategoryImportBatchStatus.Failed);
    ...
    public async Task Handle_WhenImportIsPendingReview_IsIdempotentAndDoesNotExtractAgain()
    {
        ...
        extraction.Verify(e => e.ExtractAsync<CategoryExtractionResult>(
            It.IsAny<IReadOnlyList<DocumentExtractionInput>>(), It.IsAny<CancellationToken>()), Times.Never);
    }
}
