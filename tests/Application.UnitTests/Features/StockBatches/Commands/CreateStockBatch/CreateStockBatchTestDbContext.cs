...
/// <summary>
/// Minimal <see cref="IApplicationDbContext"/> implementation for CreateStockBatch
/// handler/validator tests. Maps <see cref="StockBatch"/> and the <see cref="Item"/>/
/// <see cref="Location"/>/<see cref="SchoolClass"/>/<see cref="GoodsReceipt"/>/
/// <see cref="Category"/> entities it references, plus <see cref="UserProfile"/> for the
/// various CreatedBy/LastModifiedBy/User navigations. Mirrors GoodsReceiptTestDbContext/
/// StockBatchTestDbContext and avoids EF's "ambiguous one-to-one relationship" error the full
/// production model would trigger for UserProfile.
/// </summary>
public class CreateStockBatchTestDbContext(DbContextOptions<CreateStockBatchTestDbContext> options)
