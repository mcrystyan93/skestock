...
|-----------------|------|--------------|----------|
| `Domain` | Entities (`Category`, `CategoryIcon`, `CategoryImportBatch(+File)`, `Item`, `ItemImportBatch(+File)`, `ImportBatchHistory`, `Location`, `SchoolClass`, `ClassBalance`, `ClassItemStockVisibility`, `GoodsReceipt(Import/Line)`, `OrderList(+Line)`, `StockBatch`, `StockTransaction`, `FileMetadata`, `UserProfile`), `BaseEntity`/`BaseAuditableEntity`, domain events, queue contracts (`OutboxMessage`, `MessageEnvelope`, `ProcessedMessage`) | Any dependency on other layers, persistence/HTTP concerns | `src/Domain/Entities/*`, `src/Domain/Queues/*` |
...
| `Web` | `IEndpointGroup` classes (Categories, CategoryImportBatches, GoodsReceipts, Items, ItemImportBatches, Locations, OrderLists, SchoolClasses, Statistics, Stock, StockBatches, Storage, Users; plus a fully-commented-out `Antiforgery` group), OpenAPI/Scalar wiring, `ProblemDetailsExceptionHandler`, `OutboxPublisherService` (background job), `CurrentUser : IUser` | Direct EF Core access, business/validation logic | `src/Web/Endpoints/*.cs`, `src/Web/BackgroundJobs/OutboxPublisherService.cs` |
| `Worker` | Shared `QueueProcessingService<TProcessor>` hosted module for polling, envelope decoding, idempotency, Mediator dispatch, retry classification, and poison-queue routing; queue-specific adapters provide queue names and visibility timeouts | HTTP endpoints, ad hoc processing outside the Mediator dispatch path (`Worker.cs` template leftover excluded) | `src/Worker/Queues/QueueProcessingService.cs`, `src/Worker/Queues/*Import*QueueProcessingService.cs` |
...
| Deep queue-processing module with thin adapters | `src/Worker/Queues/QueueProcessingService.cs` plus one adapter per import queue | Keeps delivery semantics and failure handling local to one implementation while queue-specific configuration stays explicit at a small seam |
...
- **No authorization model beyond authentication** (most severe): endpoint groups authenticate by default (`RequiresAuthorization => true`), but no command/query carries `[Authorize(Roles=...)]` and `Users.cs` opts out (`RequiresAuthorization => false`) while exposing `MapIdentityApi`'s public `/register`. Authentication is enforced; *authorization* (privilege separation) is effectively absent. See `CONCERNS.md` §1 (Critical).
