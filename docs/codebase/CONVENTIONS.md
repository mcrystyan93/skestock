...
| Command/Query verbs | Still not fully standardized across slices — `Create`+`Edit`+`Disable`+`Enable` (Items) vs. `Create`+`Update` (Locations, SchoolClasses, Categories now includes `Update` too) vs. import-specific verbs (`Create*ImportBatch`, `Confirm*ImportBatch`, `Process*ImportBatch`) for the async import flows | `EditItemCommand`, `UpdateCategoryCommand`, `ConfirmGoodsReceiptImportCommand` | `src/Application/Features/*/Commands/*` |
